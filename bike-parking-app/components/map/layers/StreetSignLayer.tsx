...
import getStreetSignCoords from "@/lib/getStreetSignCoords";

const StreetSignLayer = () => {
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      const fetchData = async () => {
        setLoading(true);
        try {
          const data = await getStreetSignCoords();
          setMarkers(data);
        } catch (error) {
          console.error(error);
        }
  ...
  return (
    <>
      ...
      {!loading && (
        <>
          <LayersControl.Overlay name="Street Signs" checked>
            <LayerGroup>
              <MarkerClusterGroup
                chunkedLoading={true}
...
export default StreetSignLayer;
