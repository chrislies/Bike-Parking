...
const PruneMarkerClusterLayer = () => {
        ...
        if (coordinatesData) {
          coordinatesData.forEach((coordinate) => {
            ...
            if (coordinate.rack_type && getImageSource(coordinate.rack_type)) {
              imageSource = `
                <img class="rounded-md shadow" height=${imageSize} width=${imageSize} src="${getImageSource(coordinate.rack_type)}" alt="${coordinate.rack_type}"></img>
              `;
            } else if (coordinate.type === "sign") {
              imageSource = `
                <img class="rounded-md shadow" height=${imageSize} width=${imageSize} src="/images/streetsign.jpg" alt="street sign"></img>
              `;
            } else {
              imageSource = `
                <div class="flex flex-col w-full items-center bg-slate-200 border-[1px] border-slate-300 rounded-lg p-3">
                  ${NoImageSVG}
                  <p class="!p-0 !m-0 text-xs">No image available</p>
                </div>
              `;
            }
            ...
            marker.data.popup = `
              <div class="flex flex-col">
                <p class="!m-0 !p-0 text-base font-extrabold font-sans">${coordinate.rack_type ? coordinate.rack_type : "Street Sign"}</p>
                ${coordinate.type === 'sign' ? `<div class="relative"><div class="my-1 flex justify-center items-center select-none">${imageSource}</div><div class="absolute bottom-0 my-1 flex justify-center items-center bg-black/85 w-full h-[94%] px-1 rounded-md opacity-0 hover:opacity-100 duration-500 ease-in-out transition-all"><p class="text-xs text-white font-mono">${coordinate.sign_description}</p></div></div>`:`<div class="my-1 flex justify-center items-center select-none pointer-events-none">${imageSource}</div>`}
                <p class="text-center ${coordinate.type === 'rack' ? 'text-base overflow-x-auto' : 'max-w-[170px] text-sm tracking-tight'} hover:underline !p-0 !m-0"><a href="https://www.google.com/maps/@?api=1&map_action=pano&viewpoint=${coordinate.y},${coordinate.x}" target="_blank" class="!text-black">${coordinate.address}</a></p>
                <div class="flex flex-col gap-2 mt-1 mb-3">
                  <button
                    id="save-location-${coordinate.id}"
                    title="Save Location" aria-label="Save Location"
                    aria-disabled="false"
                    class="save-location-btn flex text-sm font-bold justify-center items-center w-full border-[1px] rounded-3xl border-slate-300 bg-slate-200 hover:bg-slate-300">
                    ${savedLocations[coordinate.id] ? SavedBookmarkSVG : BookmarkSVG}
                    Save
                  </button>
                  <button
                    id="get-directions-${coordinate.id}"
                    title="Directions" aria-label="Directions"
                    class="get-directions-btn flex text-sm font-bold justify-center items-center w-full border-[1px] rounded-3xl border-blue-600 hover:shadow-lg gap-1 text-white bg-blue-600">
                    ${DirectionsSVG}
                    Directions
                  </button>
                </div>
                ${coordinate.rack_type ? `<div class="popup dateInst"><i>Date Installed: ${formatDate(coordinate.date_inst)}</i></div>` : ""}
              </div>
            `;
          ...
          map.on("popupopen", (e) => {
            ...
            if (directionsBtn) {
              ...
              const coordinate = coordinatesData.find(
                (c) => c.id.toString() === coordinateId
              );
              if (coordinate) {
                directionsBtn.addEventListener("click", () =>
                  handleGetDirections(coordinate, directionsBtn)
                );
              }
            }
            if (saveLocationBtn) {
  ...
  const handleGetDirections = (marker) => {
    setRouteCoordinates({ x: marker.x, y: marker.y });
  };
  const handleSaveLocation = (marker) => {
    savedLocations.push(marker.id);
